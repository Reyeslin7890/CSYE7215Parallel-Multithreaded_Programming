...
public class Simulation {
    ...
    /**
     * Invarient: every event must be listed in the SimulationEvent class;
     * numCustomers, numCooks, numTables, and machineCapacity are postive integers.
     * Precondition: none
     * Postcondition: a list of simulationEvents.
     * Exception: none
     */
    public static List<SimulationEvent> runSimulation(
        ...
        Customer.setNumTable(numTables);
        ...
        if (!randomOrders) {
            ...
            for (int i = 0; i < customers.length; i++) {
                customers[i] = new Thread(
                        new Customer("Customer " + (i + 1), order, rnd.nextInt(3) + 1)
                );
            }
        } else {
            for (int i = 0; i < customers.length; i++) {
