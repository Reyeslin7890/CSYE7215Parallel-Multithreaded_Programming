...
public class AuctionServer
{
	...
	public int submitItem(String sellerName, String itemName, int lowestBiddingPrice, int biddingDurationMs)
	{
		...
		//if (serverHoldingItems>=serverCapacity) return -1;
		
		//if (sellerName not in itemsPerSeller) {make an entry for the seller}
		//check the number from itemsPerseller
		//if (theNumber >= maxSellerItems) return -1;
		
		//if (biddingDuration<=0) return -1;
		
		//if (lowestBiddingPrice>100) return -1;
		
		//make an unique itemID for the item, and list it in itemsAndIDs
		//place the item in the itemsUpForBidding
		//increment the number from itemsPerseller where the key is the seller
		//increment the number that server is holding
		//return itemId;
	...
	*/
	public List<Item> getItems()
	{
		...
		//Lock on itemsUpForBidding
		//itemsCopy = itemsUpForBidding
		//return itemsCopy
	...
	public boolean submitBid(String bidderName, int listingID, int biddingAmount)
	{
		...
		// if (highestBidders.get(listingID).equalto(bidderName)) 
		//		return false;
	
		// if (highesetBid.get(listingID) >= biddingAmount)
		//		return false;
	...
	public int checkBidStatus(String bidderName, int listingID)
	{
		...
		//if (item in items)
		//	if (item.biddingDuration+item.start >= now) 
				return 2;
		//	else 
		// 		{items.remove(item);
		//		serverHoldingItems--;
		//		itemsPerSeller.get(item.sellerName)--;
		
		//if (item not in items && item in itemsAndIDs)
		//	if (highestBidders.get(listingID).equalto(bidderName)) 
		//		{itemsPerSeller.get(bidderName)--
		//		return 1;}
		//return 3;
	}
	...
	public Boolean itemUnbid(int listingID)
	{
		...
		// lock on highestBid
		// check itemsUpForBidding size
		// if (size==0) return false;
		// for (item in highestBid)
		//		if (itemID == listingID) 
		//			return false;
